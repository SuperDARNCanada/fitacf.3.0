...
void set_w_s(llist_node range, struct FitRange* fit_range_array, FITPRMS* fit_prms){
	...
	fit_range_array[range_node->range].w_s = sqrt(fabs(range_node->q_pwr_fit->b)) * conversion_factor;
}
...
void set_w_s_err(llist_node range, struct FitRange* fit_range_array, FITPRMS* fit_prms){
	...
	fit_range_array[range_node->range].w_s_err = sqrt(range_node->q_pwr_fit_err->sigma_2_b)/2./sqrt(fabs(range_node->q_pwr_fit->b)) * conversion_factor;
}
